...
def generate_blurb(record_str, gp, pt_diff_avg, sos, preseason_rank, current_rank):
    """기록/득실차/SOS/프리시즌 대비 순위를 조합해 1~2문장 자동 요약 생성 (한글).
    실제 뉴스가 아니라 지금 갖고 있는 데이터로만 설명 가능한 근거를 서술."""
    if gp == 0:
        return f"아직 경기 기록 없음. 프리시즌 전문가 컨센서스 {preseason_rank}위를 그대로 반영 중."

    margin_txt = f"평균 {pt_diff_avg:+.1f}점차" if abs(pt_diff_avg) >= 0.5 else "박빙의 승부"

    if sos >= 0.55:
        sos_txt = "상대적으로 만만치 않은 일정을 소화했고"
    elif sos <= 0.45:
        sos_txt = "비교적 약한 상대들과 붙었고"
    else:
        sos_txt = "평이한 강도의 일정을 치렀고"

    delta = preseason_rank - current_rank  # 양수면 프리시즌 예상보다 상승
    if delta >= 5:
        expectation_txt = f"프리시즌 {preseason_rank}위 예상을 크게 뛰어넘어 현재 {current_rank}위."
    elif delta <= -5:
        expectation_txt = f"프리시즌 {preseason_rank}위 기대에 못 미쳐 현재 {current_rank}위로 처짐."
    else:
        expectation_txt = f"프리시즌 {preseason_rank}위 예상과 대체로 부합하는 {current_rank}위."

    return f"{record_str} 기록, {margin_txt}. {sos_txt}, {expectation_txt}"

...
def predict_score(home_team, away_team):
    """홈팀 평균득점과 원정팀 평균실점의 평균, 그 반대 조합으로 스코어 근사.
    정교한 확률모델이 아니라 '지금 갖고 있는 득실점 데이터로 낼 수 있는
    가장 단순하고 설명 가능한 추정치'."""
    pred_home = (home_team['avgPointsFor'] + away_team['avgPointsAgainst']) / 2
    pred_away = (away_team['avgPointsFor'] + home_team['avgPointsAgainst']) / 2
    return round(pred_home), round(pred_away)
    ...
    for row in rows:
        ...
        if game_key not in games_store:
            ...
            if not has_score and int(week) == next_week_to_predict:
                ...
                pred_home_score, pred_away_score = predict_score(by_id[home], by_id[away])
